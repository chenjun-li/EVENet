...
def options_parse():
    ...
    """
    parser = optparse.OptionParser(version='$Id:map_surf_label.py,v 1.0 2022/08/24 21:22:08 mreuter Exp $',
                                   usage=HELPTEXT)
...
def writeSurfLabel(filename, sid, label, values, surf):
    ...
    if values.size != label.size:
        raise ValueError(
            "writeLabel Error: label and values should have same sizes {}!={}".format(label.size, values.size))
    coords = surf[label, :]
...
def mapSurfLabel(src_label_name, out_label_name, trg_surf, trg_sid, rev_mapping):
    ...
    if trg_surf.shape[0] != tmax:
        raise ValueError(
            "mapSurfLabel Error: label and trg vertices should have same sizes {}!={}".format(tmax, trg_surf.shape[0]))
