...
def mode_filter(adjM, labels, fillonlylabel="", novote=[]):
    ...
    if n != adjM.shape[0] or n != adjM.shape[1]:
        sys.exit("ERROR mode_filter: adjM size " + format(adjM.shape) + " does not match label length " + format(
            labels.shape))
...
def smooth_aparc(insurfname, inaparcname, incortexname, outaparcname):
    ...
    if labels.size != surf[0].shape[0]:
        sys.exit(
            "ERROR smooth_aparc: vertec count " + format(surf[0].shape[0]) + " does not match label length " + format(
                labels.size))
    ...
    print("Non-cortex vertices with labels: " + str(
