...
threads="1"           # number of threads to use for running EVENetCNN
...
then
  ...
  binpath="$EVENet_HOME/recon_surf/"
fi
...
FLAGS:
  ...
  --t1  <T1_input>        T1 full head input (not bias corrected). This must be
                            ...
                            Images can be conformed using EVENetCNN's
                            conform.py script (usage example: python3
                            EVENetCNN/data_loader/conform.py -i <T1_input>
  ...
  --aparc_aseg_segfile <aparc_aseg_segfile>
                          Name of intermediate DL-based segmentation file
                            ...
                            orientation). EVENetCNN's segmentations are
...
then
  ...
  # Explanation, see https://github.com/Deep-MI/EVENet/issues/186
...
cmd="$python ${binpath}/../EVENetCNN/quick_qc.py --aparc_aseg_segfile $aparc_aseg_segfile"
echo $cmd
$cmd
if [ ${PIPESTATUS[0]} -ne 0 ] ; then exit 1 ; fi
echo
...
cmd="$python ${binpath}../EVENetCNN/data_loader/conform.py -i $t1 --check_only --vox_size min --verbose"
...
cmd="$python ${binpath}../EVENetCNN/data_loader/conform.py -i $aparc_aseg_segfile --check_only --vox_size $vox_size --dtype any --verbose"
RunIt "$cmd" $LF
...
if [ ! -f "$mask" ] || [ ! -f "$mdir/aseg.auto_noCCseg.mgz" ] ; then
  ...
  cmd="$python ${binpath}/../EVENetCNN/reduce_to_aseg.py -i $mdir/aparc.DKTatlas+aseg.orig.mgz -o $mdir/aseg.auto_noCCseg.mgz --outmask $mask --fixwm"
  RunIt "$cmd" $LF
fi
...
if [ "$fsaparc" == "1" ] || [ "$fssurfreg" == "1" ] ; then
  ...
  # one gyrus too far posterior, EVENetCNN's image-based segmentation does not
  # seem to do this, so we initialize the spherical registration with the better 
  # cortical segmentation from EVENetCNN, this replaces recon-all -surfreg
