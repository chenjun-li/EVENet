...
def options_parse():
    ...
    """
    parser = optparse.OptionParser(version='$Id:create_annotation.py,v 1.0 2022/08/24 21:22:08 mreuter Exp $',
                                   usage=HELPTEXT)
...
def write_annot(annot_ids, label_names, colortab_name, out_annot, append=""):
    ...
    for name_tab, name_list in zip(col_names[offset:], label_names):
        if name_tab + append != name_list:
            ...
            raise ValueError(
                "Error: name in colortable and in label lists disagree: {} != {}".format(name_tab + append, name_list))
