...
from utils.arg_types import vox_size as __vox_size, conform_to_one_mm as __conform_to_one_mm

EVENet_ROOT = path.dirname(path.dirname(path.dirname(__file__)))
...
ALL_FLAGS = {
    ...
    "aparc_aseg_segfile": __arg(
        ...
        help="Name of intermediate DL-based segmentation file (similar to aparc+aseg). "
             "When using EVENetCNN, this segmentation is already conformed, since inference "
    ...
    "lut": __arg(
        ...
        default=path.join(EVENet_ROOT, "EVENetCNN/config/EVENet_ColorLUT.tsv")),
    "allow_root": __arg(
...
def add_arguments(parser: argparse.ArgumentParser, flags: Iterable[str]) -> argparse.ArgumentParser:
    ...
    for flag in flags:
        ...
        if add_flag is not None:
            ...
            raise ValueError(f"The flag '{flag}' is not defined in EVENetCNN.utils.parse.add_arguments().")
    return parser
...
def add_plane_flags(parser: argparse.ArgumentParser, type: str, files: Mapping[str, str]) -> argparse.ArgumentParser:
    ...
    and the values is the file name (relative for path relative to EVENet_HOME.
    ...
    Args:
        ...
        files: A dictionary of plane to filename. Relative files are assumed to be relative to the EVENetCNN root
            directory.
    ...
    for key, filepath in files.items():
        if not path.isabs(filepath):
            filepath = path.join(EVENet_ROOT, filepath)
