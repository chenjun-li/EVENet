...
class DiceScore:
    ...
    def _update_union_intersection(self, batch_output, labels_batch):
        for i in range(self.n_classes):
            ...
            # if (torch.max(gt) > 1):
            #     print("err1", torch.max(gt))
            # if (torch.max(pred) > 1):
            #     print("err2", torch.max(pred))
            # if(i == 0):
            #     print("mul:",torch.sum(torch.mul(gt, pred)))
            #     print("pred:",torch.sum(pred))
            #     print("gt:",torch.sum(gt))
            #     tep = torch.sum(torch.mul(gt, pred))*2 - torch.sum(pred) - torch.sum(gt)
            #     print("tep",tep)
            #     print()
            # if torch.sum(torch.mul(gt, pred)) * 2 > (torch.sum(gt) + torch.sum(pred)):
            #     print("err3",i)
    ...
    def update(self, output, cnf_mat):
        ...
        ## comment out the following lines in stage 2
        if cnf_mat:
    ...
    def _dice_calculation(self):
        # dsc = np.zeros(self.n_classes)
        # for i in range(self.n_classes):
        #     inter = 2 * self.intersection[i,i]
        #     uni = self.union[i,i]
        #     dsc[i] = inter / uni
