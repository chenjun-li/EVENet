...
from data_loader.dataset import Fusion_MultiScaleOrigDataThickSlices
...
class Inference:
    ...
    def setup_model(self, cfg=None, device: torch.device = None):
        ...
        # Set up model
        self._model_not_init = build_model(self.cfg)  # ~ model = EVENetCNN(params_network)
    ...
    @torch.no_grad()
    def eval(self, init_pred: torch.Tensor, val_loader: DataLoader, *, out_scale=None,
        ...
        with logging_redirect_tqdm():
            try:
                for batch_idx, batch in tqdm(enumerate(val_loader), total=len(val_loader), unit="batch"):
                    ...
                    pred = self.model(images, scale_factors, out_scale).cpu()
                    # print("initial", pred.shape)
                    # image_2 = pred.cpu().numpy()
                    # np.ptp(image_2)
    ...
    @torch.no_grad()
    def run_mix(self, init_pred: torch.Tensor, img_filename, orig_data, orig_data1, orig_data2, orig_zoom,
        ...
        test_dataset = Fusion_MultiScaleOrigDataThickSlices(img_filename, orig_data, orig_data1, orig_data2, orig_zoom,
                                                            self.cfg,
                                                            transforms=transforms.Compose([ToTensorTest()]))
