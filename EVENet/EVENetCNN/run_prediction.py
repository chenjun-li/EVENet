...
class RunModelOnData:
    ...
    def conform_and_save_orig(self, orig_str: str) -> Tuple[nib.analyze.SpatialImage, np.ndarray]:
        ...
        self.save_img(self.subject_conf_name, orig_data, orig, dtype=np.float32)
    ...
    def get_prediction(self, orig_f: str, orig_data: np.ndarray, zoom: Union[np.ndarray, tuple]) -> np.ndarray:
        ...
        # torch.save(pred_prob, 'pred_prob_e3.pt')
        # print("Tensor shape:", pred_prob.shape)
        pred_classes = torch.argmax(pred_prob, 3)
        pred_value = pred_prob.float()

        # Calculate softmax entropy
        pred_prob = pred_prob.float()
        probs = torch.nn.functional.softmax(pred_prob, dim=3)
        del pred_prob
        torch.max(probs)
        torch.min(probs)
        entropy = -torch.sum(probs * torch.log2(probs + 1e-6), dim=3)
        min_entropy = entropy.min()
        max_entropy = entropy.max()
        norm_entropy = (entropy - min_entropy) / (max_entropy - min_entropy)

        # Create heatmap

        voxel_size = (0.7, 0.7, 0.7)
        heatmap = norm_entropy.cpu().numpy()
        heatmap = heatmap.astype(np.float32)
        affine = np.eye(4)
        nifti_image = nib.Nifti1Image(heatmap, affine)
        nifti_image.header.set_zooms(voxel_size)
        # nib.save(nifti_image, 'heatmap.nii.gz')

        # zd = nib.Nifti1Image(pred_value.numpy(), affine=None)
        # nib.save(zd, '/csnzd.nii.gz')

        # plt.imshow(norm_entropy_np, cmap='inferno')
        # plt.colorbar()
        # plt.savefig('uncertainty_heatmap.png')
        # np_dbg = pred_classes[120].cpu().numpy()
        # np.savetxt('dbg.txt', np_dbg)
        # np.count_nonzero(np_dbg)
        ...
        # return pred_value, norm_entropy # for stage 2
        return pred_classes, norm_entropy

    def get_prediction_evidential(self, orig_f: str, orig_data: np.ndarray,
                                  zoom: Union[np.ndarray, tuple]) -> np.ndarray:
        shape = orig_data.shape + (self.get_num_classes(),)
        kwargs = {
            "device": self.viewagg_device,
            "dtype": torch.float16,
        ...
        torch.save(pred_prob, 'pred_prob_XX.pt')
        # print("Tensor shape:", pred_prob.shape)
        pred_classes = torch.argmax(pred_prob, 3)

        sum_probabilities = torch.sum(pred_prob, dim=-1)
        uncertainty = 1 - sum_probabilities
        ...
        nifti_image = nib.Nifti1Image(heatmap, affine)
        nifti_image.header.set_zooms(voxel_size)
        nib.save(nifti_image, 'heatmap_ce.nii.gz')

        # map to freesurfer label space
        pred_classes = du.map_label2aparc_aseg(pred_classes, self.labels)
        # return numpy array TODO: split_cortex_labels requires a numpy ndarray input
        pred_classes = du.split_cortex_labels(pred_classes.cpu().numpy())
        return pred_classes, uncertainty

    def get_prediction_stacked(self, orig_f: str, orig_data: np.ndarray, orig_data1: np.ndarray, orig_data2: np.ndarray,
                               zoom: Union[np.ndarray, tuple]) -> np.ndarray:
        # Stack the data along the specified axis
        # stacked_data = np.concatenate([orig_data, orig_data1, orig_data2], axis=1)
        ...
        for plane, model in self.models.items():
            ...
            self.set_model(plane)
            # pred_prob is updated inplace to conserve memory
            pred_prob = model.run(pred_prob, orig_f, orig_data, orig_data1, orig_data2, zoom, out=pred_prob)
...
if __name__ == "__main__":
    ...
    parser = parser_defaults.add_plane_flags(parser, "config",
                                             {"coronal": "EVENetCNN/config/EVENetVINN_coronal.yaml",
                                              "axial": "EVENetCNN/config/EVENetVINN_axial.yaml",
                                              "sagittal": "EVENetCNN/config/EVENetVINN_sagittal.yaml"})
    ...
    if not args.allow_root and os.name == 'posix' and os.getuid() == 0:
        sys.exit(
            ...
            EVENetCNN as root, because it will lead to files and folders created as root.
            If you are running EVENetCNN in a docker container, you can specify the user with 
    ...
    for subject in s_dirs:
        ...
        heatmap_name = os.path.join(out_dir, sbj_name, 'heatmap.nii.gz')

        # Run model
        try:

            # pred_data, heatmap_data = eval.get_prediction(orig_fn, data_array, orig_img.header.get_zooms())
            # pred_data = eval.get_prediction_stacked(orig_fn, md_array, e3_array, fa_array, orig_img.header.get_zooms())
            pred_data, heatmap_data = eval.get_prediction_evidential(orig_fn, data_array, orig_img.header.get_zooms())

            eval.save_img(pred_name, pred_data, orig_img, dtype=np.int16)
            # eval.save_img(pred_name, pred_data, orig_img, dtype=np.float32) #for uncertainty output
            eval.save_img(heatmap_name, heatmap_data, orig_img, dtype=np.float32)
