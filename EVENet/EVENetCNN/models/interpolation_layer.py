...
from utils.logging import getLogger as _getLogger
...
class _ZoomNd(nn.Module):
    ...
    def _calculate_crop_pad(self,
                            ...
                            dim: int, alignment: str) -> _T.Tuple[
        slice, T_Scale, _T.Union[bool, _T.Tuple[int, int]], int]:
        """
...
class Zoom3d(_ZoomNd):
    ...
    def _interpolate(self, tensor: Tensor, scale_factor: _T.Sequence[int]):
        ...
        front_back, scale_factor, pad_fb, shape_fb = self._calculate_crop_size(tensor.shape, scale_factor, 0,
                                                                               depth_alignment)
        top_bottom, scale_factor, pad_tb, shape_tb = self._calculate_crop_pad(tensor.shape, scale_factor, 1,
                                                                              vertical_alignment)
        left_right, scale_factor, pad_lr, shape_lr = self._calculate_crop_pad(tensor.shape, scale_factor, 2,
                                                                              horizontal_alignment)
        needs_padding = pad_tb or pad_lr or pad_fb
        interp = _F.interpolate(tensor[:, :, front_back, top_bottom, left_right], size=(shape_fb, shape_tb, shape_lr),
                                mode=self._mode)  # , align_corners=False)
