...
class EvidentialLoss(nn.Module):
    ...
    def forward(self, inputx, target, weight):
        ...
        alpha = input_soft * weight.unsqueeze(1)
        beta = weight.unsqueeze(1) - alpha + 1.0

        # Sample from Beta distribution
        beta_dist = torch.distributions.beta.Beta(alpha, beta)
        samples = beta_dist.rsample(torch.Size([100]))  # Sample 100 times

        # Compute loss as negative log-likelihood of target
        negative_log_likelihood = -beta_dist.log_prob(target.unsqueeze(1))
...
def get_loss_func(cfg):
    if cfg.MODEL.LOSS_FUNC == 'combined':
        ...
        return DiceLoss()
    # elif cfg.MODEL.LOSS_FUNC == "evidential":
