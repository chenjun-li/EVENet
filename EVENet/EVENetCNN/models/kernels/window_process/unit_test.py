...
class Test_WindowProcess(unittest.TestCase):
    ...
    def test_roll_and_window_partition_forward(self, dtype=torch.float32):
        ...
        with torch.no_grad():
            ...
            # fused kernel
            fused_output = WindowProcess.apply(input2, self.B, self.H, self.W, self.C, -self.shift_size,
                                               self.window_size)
    ...
    def test_roll_and_window_partition_backward(self, dtype=torch.float32):
        input = torch.randn((self.B, self.H, self.W, self.C), dtype=dtype, requires_grad=True).cuda()
        d_loss_tensor = torch.randn((self.B * self.nW * self.nH, self.window_size, self.window_size, self.C),
                                    dtype=dtype).cuda()
    ...
    def test_window_merge_and_roll_forward(self, dtype=torch.float32):
        input = torch.randn((self.B * self.nH * self.nW, self.window_size, self.window_size, self.C), dtype=dtype,
        ...
        with torch.no_grad():
            ...
            # fused kernel
            fused_output = WindowProcessReverse.apply(input2, self.B, self.H, self.W, self.C, self.shift_size,
                                                      self.window_size)
