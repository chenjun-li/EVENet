...
if [ -z "$EVENet_HOME" ]
then
  echo "Setting ENV variable EVENet_HOME to current working directory ${PWD}. "
  echo "Change via enviroment to location of your choice if this is undesired (export EVENet_HOME=/dir/to/EVENet)"
  export EVENet_HOME=${PWD}
fi
EVENetcnndir="$EVENet_HOME/EVENetCNN"
reconsurfdir="$EVENet_HOME/recon_surf"
...
Usage: run_EVENet.sh --sid <sid> --sd <sdir> --t1 <t1_input> [OPTIONS]

run_EVENet.sh takes a T1 full head image and creates:
     (i)  a segmentation using EVENetVINN (equivalent to FreeSurfer
...
FLAGS:
  ...
  --seg_only              Run only EVENetVINN (generate segmentation, do not
                            run surface pipeline)
  --seg_log <seg_log>     Log-file for the segmentation (EVENetVINN)
  ...
  --main_segfile <filename>
                          Name of the segmentation file, which includes all labels
                            ...
                            EVENet, this segmentation is already conformed,
                            ...
                            Requires an ABSOLUTE Path! Default location:
                            \$SUBJECTS_DIR/\$sid/mri/EVENet.merged.mgz
...
Henschel L, Conjeti S, Estrada S, Diers K, Fischl B, Reuter M, EVENet - A
...
Henschel L*, Kuegler D*, Reuter M. (*co-first). EVENetVINN: Building
...
if [ -z "$allow_root" ] && [ "$(id -u)" == "0" ]
  then
    echo "You are trying to run '$0' as root. We advice to avoid running EVENet as root, "
    echo "because it will lead to files and folders created as root."
    echo "If you are running EVENet in a docker container, you can specify the user with "
    echo "'-u \$(id -u):\$(id -g)' (see https://docs.docker.com/engine/reference/run/#user)."
    echo "If you want to force running as root, you may pass --allow_root to run_EVENet.sh."
    exit 1;
fi
...
if [ -z "$main_segfile" ]
  then
    main_segfile="${sd}/${subject}/mri/EVENet.merged.mgz"
fi
...
# make sure EVENetCNN is in the PYTHONPATH
if [ "$PYTHONPATH" == "" ]
then
  export PYTHONPATH="$EVENet_HOME"
else
  export PYTHONPATH="$EVENet_HOME:$PYTHONPATH"
fi
...
if [ "$run_surf_pipeline" == "0" ] && [ "$run_seg_pipeline" == "0" ]
  then
    ...
    echo "To run the whole EVENet pipeline, omit both flags."
    exit 1;
fi
...
if [ "$run_seg_pipeline" == "1" ]
  then
    # "============= Running EVENetCNN (Creating Segmentation aparc.DKTatlas.aseg.mgz) ==============="
    # use EVENetCNN to create cortical parcellation + anatomical segmentation into 95 classes.
    mkdir -p "$(dirname "$seg_log")"
    echo "Log file for segmentation EVENetCNN/run_prediction.py" > $seg_log
    date  |& tee -a $seg_log
    echo "" |& tee -a $seg_log

    cmd="$python $EVENetcnndir/run_prediction.py --t1 $t1 --aparc_aseg_segfile $aparc_aseg_segfile --conformed_name $conformed_name --sid $subject --seg_log $seg_log --vox_size $vox_size --batch_size $batch_size --viewagg_device $viewagg --device $device $allow_root"
...
if [ "$run_surf_pipeline" == "1" ]
  then
    ...
    # use recon-surf to create surface models based on the EVENetCNN segmentation.
    pushd $reconsurfdir
